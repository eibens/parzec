...
par.Parser.prototype.oneOrMoreSeparatedBy = function <T, U, S>(
  this: par.Parser<T, S>,
  separator: par.Parser<U, S>,
): par.Parser<T[], S> {
  return this.bind(
    (x) =>
      separator.seq(this).zeroOrMore().bind(
 ...
 */
par.Parser.prototype.followedBy = function <T, U, S>(
  this: par.Parser<T, S>,
  after: par.Parser<U, S>,
): par.Parser<T, S> {
 ...
 */
par.Parser.prototype.surroundedBy = function <T, U, S>(
  this: par.Parser<T, S>,
  surround: par.Parser<U, S>,
): par.Parser<T, S> {
 ...
 * is returned.
 */
par.Parser.prototype.bracketedBy = function <T, U, V, S>(
  this: par.Parser<T, S>,
  open: par.Parser<U, S>,
  close: par.Parser<V, S>,
): par.Parser<T, S> {
  return open.bind(
    () =>
      this.bind(
 ...
 * expression grammars.
 */
par.Parser.prototype.chainOneOrMore = function <T, S>(
  this: par.Parser<T, S>,
  operation: par.Parser<BinaryOp<T>, S>,
): par.Parser<T, S> {
  return this.bind(
    (x) =>
      operation.bind(
        (f) =>
          this.bind(
            (y) => par.mret([f, y] as [BinaryOp<T>, T]),
          ),
      ).zeroOrMore().bind(
 ...
 */
par.Parser.prototype.chainZeroOrMore = function <T, S>(
  this: par.Parser<T, S>,
  operation: par.Parser<BinaryOp<T>, S>,
  value: T,
): par.Parser<T, S> {
 ...
 */
export function operators<T, U, S>(
  ...ops: [par.Parser<T, S>, U][]
): par.Parser<U, S> {
