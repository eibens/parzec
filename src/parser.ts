...
export class Parser<T, S> {
  ...
  bind<U>(binder: (value: T) => Parser<U, S>): Parser<U, S> {
    return new Parser((input) => {
      ...
      if (res1.kind == "ok") {
        const res2 = binder(res1.result).parse(input);
  ...
  or<U>(other: Parser<U, S>): Parser<T | U, S> {
    return new Parser((input) => {
      const pos = input.position;
      const res1 = this.parse(input) as pr.ParseResult<T | U>;
      if (res1.kind == "ok") {
        return res1;
      }
  ...
  zeroOrMore(): Parser<T[], S> {
    return new Parser((input) => {
      ...
      while (true) {
        ...
        if (res.kind == "fail") {
          return res.position > pos ? res : pr.succeeded(res.position, list);
        }
  ...
  expect(expected: string): Parser<T, S> {
    ...
    const resParser = new Parser((input: inp.ParserInput<S>) => {
      const res = this.parse(input);
      if (res.kind == "fail") {
...
export const parserDebug = {
   ...
   */
  write(text: string) {
    const tabs = "  ".repeat(this.indentation);
...
export function parse<T, S>(
  ...
  const res = tryParse(parser, input);
  if (res.kind == "fail") {
    throw new err.ParseError(
      err.ErrorSource.Parser,
      res.position,
      res.found,
      res.expected,
    );
