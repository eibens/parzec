...
// deno-lint-ignore no-explicit-any
const littrue = pz.terminal(JsonToken.True, "true").map(() => <any> true)
  .followedBy(whitespace);
// deno-lint-ignore no-explicit-any
const litfalse = pz.terminal(JsonToken.False, "false").map(() => <any> false)
  .followedBy(whitespace);
// deno-lint-ignore no-explicit-any
const litnull = pz.terminal(JsonToken.Null, "null").map(() => <any> null)
  .followedBy(whitespace);
...
const beginarray = pz.terminal(JsonToken.LeftBracket, "[").followedBy(
  whitespace,
);
const endarray = pz.terminal(JsonToken.RightBracket, "]").followedBy(
  whitespace,
);
const beginobject = pz.terminal(JsonToken.LeftBrace, "{").followedBy(
  whitespace,
);
...
element.target = pz.choose(
  (token: pz.Token<JsonToken>) => {
    switch (token.token) {
      ...
      default:
        // deno-lint-ignore no-explicit-any
        return pz.fail<any, pz.Token<JsonToken>>(
          token.text,
          "{",
          "[",
          "<string>",
...
export function jsonInput(text: string): pz.ParserInput<pz.Token<JsonToken>> {
  return pz.lexerInput<JsonToken>(
    text,
    lexer,
